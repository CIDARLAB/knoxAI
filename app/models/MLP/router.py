...
from app.models.XGBoost.config import Config as XGBConfig

router = APIRouter(prefix="/mlp", tags=["MLP"])
...
def train_endpoint(request: MLPTrainRequest):
...
def predict_endpoint(request: MLPPredictRequest):
    model = MLPModel(config=Config(**request.config))
...
def tune_endpoint(request: MLPTuneRequest):
    request.config["vocab_size"] = request.vocab_size
    request.config["task"] = request.task
